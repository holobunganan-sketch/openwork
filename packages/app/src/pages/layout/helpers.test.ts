...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openwork://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("keeps manually supplied OpenCode links compatible without registering its protocol", () => {
    expect(parseDeepLink("opencode://open-project?directory=/tmp/legacy")).toBe("/tmp/legacy")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openwork://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openwork://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openwork://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openwork://open-project")).toBeUndefined()
    expect(parseDeepLink("openwork://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openwork://open-project?directory=/a",
      "openwork://other?directory=/b",
      "openwork://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openwork://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openwork://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openwork://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openwork://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openwork://new-session?directory=/a",
      "openwork://open-project?directory=/b",
      "openwork://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: ["openwork://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openwork://open-project?directory=/a"])
