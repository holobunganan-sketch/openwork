...
2. Start an RC from the tested merge with either supported control event:
   - Push a `release-022-rcN` branch at the tested merge.
   - For GitHub App/API clients whose ref writes do not trigger Actions, create a same-repository `release-022-rcN` branch, add only `.github/openwork-release-control`, and open a pull request to `openwork-main`. The workflow releases the already-merged base commit, never the unmerged control commit.
     The release workflow derives `openwork-v0.2.2-rc.N`, rebuilds that version, runs lint, typechecking, unit tests, the production harness browser gate, the desktop build, Windows packaging, portable launch, silent NSIS install, installed launch, coexistence-marker verification, and silent uninstall. Only after every gate passes does it create the immutable RC tag. Successful control branches are removed, and control pull requests are closed automatically.
...
4. After an RC passes without release-blocking defects, use `release-022` at the same or a later tested `openwork-main` commit through either the branch-push or control-PR event. The workflow requires a published 0.2.2 RC, repeats every gate, then creates `openwork-v0.2.2` and publishes it as the latest release.

## Artifact policy

The supported 0.2.2 deliverables are Windows 11 x64 NSIS and portable executables, Electron update metadata, SHA-256 checksums, and a CycloneDX 1.6 SBOM. Artifacts are generated from the release commit and uploaded by GitHub Actions; locally built binaries are not release inputs.
